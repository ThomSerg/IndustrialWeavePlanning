...
from .variables.CreelPacking import CreelPacking
from .models.CreelPackingModel import CreelPackingModel
...
class CreelModel:
    ...
    def __init__(self,
                    ...
                    free_single_bin_packings: list[SingleBinPacking],
                    fixed_single_bin_packings: list[SingleBinPacking],
    ...
    def get_constraints(self):
        ...
        self.creel_packing = CreelPacking(
            _max_creel_number=self.max_creel_number,
            _max_deadline=self.max_deadline,
            _max_colors=self.max_colors,
            _colors=self.colors,
            _min_widths=min_widths,
            _machine_config=self.machine_config,
            )
        ...
        # A creel section cannot start or end in the mids of a bin TODO herformuleren zodat het niet gebonden is van de start van een bin maar ook in een lege ruimte kan starten
        # -> a section start must always overlap with a bin start
        # -> end is implicit, as it is the start of the next section

        # => al later gedaan door te zeggen dat elke binsection volledig in een color section moet liggen
        ...
        #         for i_bin,(bin_active,bin_start,bin_end,bin_packing) in enumerate(zip(self.bin_production.bin_active[:,i_deadline], bin_starts[:,i_deadline], bin_ends[:,i_deadline], self.bin_production._bin_packings)):
        #             d.append(
        #                 (bin_active) & (bin_start == section_start) # An active bin with the same start as the creel section
        #                 #(bin_active) & (bin_start < section_start) & (section_start <= bin_end)
        #             )

        #     c.append(
        #         (i_section < self.creel_packing.count).implies( # Active creel section
        #             any( d ) # An active bin with the same start as the creel section
        #             #~any(d)
        #         )
        #     )
        ...
        #     # c.append(
        #     #     (i_section < self.creel_packing.count).implies( # Active creel section
        #     #         any( (self.bin_production.bin_active) & (bin_starts == self.creel_packing.starts) ) # An active bin with the same start as the creel section
        #     #     )
        #     # )
        ...
        #print("bin_color_ranges", bin_color_ranges)
            ...
            for bin_packing,bin_active,bin_start,bin_end in zip(self.bin_production._free_bin_packings,self.bin_production.free_bin_active[:,i_deadline], self.bin_production.bin_starts[-len(self.bin_production.free_bin_active):,i_deadline],self.bin_production.bin_ends[-len(self.bin_production.free_bin_active):,i_deadline]):
                for i_section,(section,section_start,section_end) in enumerate(zip(self.creel_packing.sections, self.creel_packing.starts, self.creel_packing.ends)):
                    ...
                    for item in bin_packing.items:
                        ...
                        # for (i_instance,pos_x) in enumerate(item.pos_xs):
                        #     ccc = []
                        #     for color in item.item.color.basic_colors:
                        #         ccc.append(
                        #             section.color_sections[section._colors.index(color)].is_here_2(pos_x,pos_x+item.width)
                        #         )
                        #     cc.append(
                        #         (item.active[i_instance]).implies(  # Active item instance
                        #             cpm_all(ccc)
                        #         )
                        #     )
                        ...
                        if item.has_fixed_packing():
                            ...
                            for color in item.item.color.basic_colors:
                                ...
                                for c_range in c_ranges:
                                    #print(section.color_sections[section._colors.index(color)].is_here_2_fixed(c_range[0],c_range[1]))
                                    cc.append(
    ...
    def color_range_from_item(self, item: ItemPacking):
        ...
        # starts = item.pos_xs[:item.count]
        # ends = item.pos_xs[:item.count] + item.widths[:item.count]
        # ranges = list((s,e) for (s,e) in zip(starts,ends,actives))
        # color = item.item.color
        # color_ranges = {}
        # for basic_color in color.basic_colors:
        #     color_ranges[basic_color] = ranges
