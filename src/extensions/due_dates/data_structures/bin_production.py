...
class BinProduction:
    ...
    bin_delays_before: NDVarArray[boolvar] = None          # [bin_solution x deadline]
    bin_delays_after: NDVarArray[boolvar] = None           # [bin_solution x deadline]
    ...
    def __post_init__(self):
        ...
        if self.deadline_betweens is None: self.deadline_betweens = np.insert(np.ediff1d(self.deadlines), 0, self.deadlines[0])
    ...
    def _bin_delays_before_var(self):
        return intvar(0, self.deadlines[-1], shape=(self.nr_packings, self.nr_deadlines)) # TODO kan strikter
